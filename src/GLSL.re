...
let genericmath2 = (l, r: trT(glslVariantTypeUBT('a)), e) =>
  switch (l, r) {
...
let floatfun: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Fun, Float, name)));

let floatattr: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Attr, Float, name)));

let floatvarying: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Varying, Float, name)));

let floatbuiltin: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Builtin, Float, name)));

let floatuniform: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Uniform, Float, name)));

let floatvar: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Var, Float, name)));

let floatoutput: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Output, Float, name)));

let floatarg: 'a => trT([ | `Float]) =
  name => Typed(`Float, RVar((Arg, Float, name)));

let vec2fun: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Fun, Vec2, name)));

let vec2attr: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Attr, Vec2, name)));

let vec2varying: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Varying, Vec2, name)));

let vec2builtin: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Builtin, Vec2, name)));

let vec2uniform: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Uniform, Vec2, name)));

let vec2var: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Var, Vec2, name)));

let vec2output: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Output, Vec2, name)));

let vec2arg: 'a => trT([ | `Vec2]) =
  name => Typed(`Vec2, RVar((Arg, Vec2, name)));

let vec3fun: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Fun, Vec3, name)));

let vec3attr: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Attr, Vec3, name)));

let vec3varying: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Varying, Vec3, name)));

let vec3builtin: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Builtin, Vec3, name)));

let vec3uniform: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Uniform, Vec3, name)));

let vec3var: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Var, Vec3, name)));

let vec3output: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Output, Vec3, name)));

let vec3arg: 'a => trT([ | `Vec3]) =
  name => Typed(`Vec3, RVar((Arg, Vec3, name)));

let vec4fun: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Fun, Vec4, name)));

let vec4attr: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Attr, Vec4, name)));

let vec4varying: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Varying, Vec4, name)));

let vec4builtin: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Builtin, Vec4, name)));

let vec4uniform: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Uniform, Vec4, name)));

let vec4var: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Var, Vec4, name)));

let vec4output: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Output, Vec4, name)));

let vec4arg: 'a => trT([ | `Vec4]) =
  name => Typed(`Vec4, RVar((Arg, Vec4, name)));

let mat2fun: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Fun, Mat2, name)));

let mat2attr: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Attr, Mat2, name)));

let mat2varying: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Varying, Mat2, name)));

let mat2builtin: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Builtin, Mat2, name)));

let mat2uniform: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Uniform, Mat2, name)));

let mat2var: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Var, Mat2, name)));

let mat2output: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Output, Mat2, name)));

let mat2arg: 'a => trT([ | `Mat2]) =
  name => Typed(`Mat2, RVar((Arg, Mat2, name)));

let mat3fun: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Fun, Mat3, name)));

let mat3attr: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Attr, Mat3, name)));

let mat3varying: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Varying, Mat3, name)));

let mat3builtin: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Builtin, Mat3, name)));

let mat3uniform: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Uniform, Mat3, name)));

let mat3var: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Var, Mat3, name)));

let mat3output: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Output, Mat3, name)));

let mat3arg: 'a => trT([ | `Mat3]) =
  name => Typed(`Mat3, RVar((Arg, Mat3, name)));

let mat4fun: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Fun, Mat4, name)));

let mat4attr: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Attr, Mat4, name)));

let mat4varying: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Varying, Mat4, name)));

let mat4builtin: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Builtin, Mat4, name)));

let mat4uniform: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Uniform, Mat4, name)));

let mat4var: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Var, Mat4, name)));

let mat4output: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Output, Mat4, name)));

let mat4arg: 'a => trT([ | `Mat4]) =
  name => Typed(`Mat4, RVar((Arg, Mat4, name)));
